...
from datos import PAISESCEN, PJS
...
def obtener_ruta(ruta_relativa):
    ...
    return os.path.join(base_path, ruta_relativa) #temas de pasar el programa pyinstaller

# Diccionarios para las banderas
BANDERAS_DISPLAY = {"Venezuela": "ven",
    "Colombia": "col",
    "Ecuador": "ecu",
    "Guyana": "guy",
    "Surinam": "sur",
...
def ejecutar_actualizacionCEN(master_frame, callback_volver):
    jugadores = []
    ...
    def cargar_archivo():
        ...
        try:
            ...
            if os.path.exists("players.txtCEN"):
                with open("players.txtCEN", "r", encoding="utf-8") as f:
                    for linea in f:
                        linea = linea.strip()
    ...
    def guardar_cambios():
        ...
        # 2. Guardamos al disco (UTF-8 es clave)
        try:
            with open("players.txtCEN", "w", encoding="utf-8") as f:
                for j in jugadores:
    ...
    def eliminar_jugador():
        ...
        if messagebox.askyesno("Confirmar", f"¿Estás seguro de que quieres eliminar a {nombre_original}?"):
            ...
            try:
                ...
                refrescar_buscador() # Esto cargará al primero de la lista
            except Exception as e:
